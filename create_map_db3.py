...
con = sqlite3.connect('hxh20170122.db')
cursor = con.cursor()
...
for z in range(14, 18):
    print('z',  z)
    # for file in files:
    for x in os.listdir(path + loc + '/Z' + str(z)):
        print('x', x)
        for y in os.listdir(path + loc + '/Z' + str(z) + '/' + x):
            ...
            y = y.strip('.png')
            print('y', y)
            with open(os.path.join(path, loc, 'Z' + str(z), x, y+'.png'), 'rb') as f:
                content = f.read()
                sql = f"INSERT INTO street (tileZ,tileX,tileY,image) VALUES (?,?,?,?);"
                print(sql)
                cursor.execute(sql, (z, y, x, sqlite3.Binary(content)))
                con.commit()
